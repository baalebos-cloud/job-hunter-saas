...
SLATE_200  = colors.HexColor("#e2e8f0")  # Borders/dividers
EMERALD    = colors.HexColor("#059669")  # Accent color (company names, score badge)
BLUE_600   = colors.HexColor("#2563eb")  # Links/highlights
SKY_50     = colors.HexColor("#f0f9ff")  # Skill pill background
SKY_200    = colors.HexColor("#bae6fd")  # Skill pill border
...
class PremiumResume:
    ...
    def render_skills(self, skills: list):
        """Render skills as professional pill badges in columns."""
        if not skills:
            return
        ...
        # Render as wrapped rows with bullet separators (cleaner than pills for ATS)
        row_skills = []
        row_width = 0
        max_width = TW - 20
